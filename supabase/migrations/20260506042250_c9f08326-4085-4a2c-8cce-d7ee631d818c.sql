...
DROP POLICY IF EXISTS "Public can view events via public view" ON public.events;

-- Recreate view without security_invoker so it can serve anon users
-- (they have no SELECT policy on events). This is safe because the
-- view exposes only non-sensitive presentation fields.
