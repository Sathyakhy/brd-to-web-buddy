...
export function resolveEventContent(
  ...
  if (!dualConfig || !dualConfig.enabled || language === "km") {
    ...
    if (Array.isArray(baseWithKm.agenda_days)) {
      baseWithKm.agenda_days = baseWithKm.agenda_days.map((day: any) => ({
        ...day,
        title: day.title_km || day.title,
        items: Array.isArray(day.items)
          ? day.items.map((it: any) => ({
              ...it,
              label: it.label_km || it.label,
              description: it.description_km ?? it.description,
              subHeader: it.subHeader_km ?? it.subHeader,
            }))
          : day.items,
      }));
    }

    // Resolve contacts for Khmer
    if (Array.isArray(baseWithKm.contacts)) {
      baseWithKm.contacts = baseWithKm.contacts.map((c: any) => ({
        ...c,
        label: c.label_km || c.label,
      }));
    }
  ...
  if (en.venue?.trim()) {
    const origParts = (event.venue ?? "").split("|");
    const link = origParts.slice(1).join("|");
    resolvedVenue = link ? `${en.venue.trim()}|${link}` : en.venue.trim();
  }
