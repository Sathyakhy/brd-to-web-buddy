      ...
      <div className="flex items-center gap-2 p-3">
        ...
        <button type="button" onClick={onToggle} className="flex-1 text-left">
          <div className="font-medium text-sm flex items-center gap-2">
            ...
            {isDual && day.title_en && (
              <span className="text-xs text-muted-foreground font-normal">/ {day.title_en}</span>
            )}
          </div>
            ...
            {isDual ? (
              <div className="space-y-1.5 sm:col-span-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-semibold uppercase text-gold bg-gold/10 px-1.5 py-0.5 rounded">Bilingual</span>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 p-2 rounded-md border border-border/80 bg-secondary/30">
                  <div className="space-y-1">
                    <span className="text-[11px] font-medium text-foreground/80 flex items-center gap-1">
                      <span>🇰🇭</span> Khmer Header
                    </span>
                    <Input
                      value={day.title}
                      onChange={(e) => onUpdate({ title: e.target.value, title_km: e.target.value })}
                      placeholder="កម្មវិធីពេលព្រឹក"
                      className="font-khmer-siemreap text-xs"
                    />
                  </div>
                  <div className="space-y-1">
                    <span className="text-[11px] font-medium text-foreground/80 flex items-center gap-1">
                      <span>🇬🇧</span> English Header
                    </span>
                    <Input
                      value={day.title_en ?? ""}
                      onChange={(e) => onUpdate({ title_en: e.target.value })}
                      placeholder="Morning Program / Ceremony"
                      className="text-xs"
                    />
                  </div>
                </div>
          ...
          <>
            ...
            <div className="sm:col-span-12 space-y-1 pt-1">
              <div className="flex items-center justify-between h-4">
                <Label className="text-[10px] uppercase tracking-wider text-muted-foreground">Title (Bilingual)</Label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div className="space-y-1">
                  <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                    <span>🇰🇭</span> Khmer Title
                  </span>
                  <Input
                    value={item.label}
                    onChange={(e) => onUpdate({ label: e.target.value, label_km: e.target.value })}
                    placeholder="ចំណងជើងកម្មវិធី (ខ្មែរ)"
                    className="h-8 font-khmer-siemreap text-xs"
                  />
                </div>
                <div className="space-y-1">
                  <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                    <span>🇬🇧</span> English Title
                  </span>
                  <Input
                    value={item.label_en ?? ""}
            ...
            <div className="sm:col-span-12 space-y-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <Input
                  ...
                  onChange={(e) => onUpdate({ subHeader: e.target.value || null, subHeader_km: e.target.value || null })}
                  placeholder="🇰🇭 e.g. ពេលព្រឹក / ពេលល្ងាច"
                  className="h-8 font-khmer-siemreap text-xs"
                />
                <Input
                  value={item.subHeader_en ?? ""}
                  onChange={(e) => onUpdate({ subHeader_en: e.target.value || null })}
            ...
            <div className="sm:col-span-12 space-y-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <Textarea
                  ...
                  onChange={(e) => onUpdate({ description: e.target.value, description_km: e.target.value })}
                  placeholder="🇰🇭 ការពិពណ៌នា (ខ្មែរ)"
                  className="font-khmer-siemreap text-xs"
                />
                <Textarea
                  rows={2}
