...
export default function ContactsEditor({ contacts, onChange }: Props) {
      ...
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={onDragEnd}>
        <SortableContext items={contacts.map((c) => c.id)} strategy={verticalListSortingStrategy}>
          <div className="space-y-2">
            {contacts.map((c) => (
              <SortableContact key={c.id} contact={c} onUpdate={(p) => update(c.id, p)} onRemove={() => remove(c.id)} />
            ))}
          </div>
        </SortableContext>
      </DndContext>
