...
type Props = {
  ...
  uploading: boolean;
  /** Whether autoplay is enabled on the cover screen (default true) */
  autoPlayCover?: boolean;
  /** Whether autoplay is enabled on the invitation page (default true) */
  autoPlayInvitation?: boolean;
  /** Callback when autoplay settings are modified */
  onAutoPlayChange?: (settings: { autoPlayCover: boolean; autoPlayInvitation: boolean }) => void;
};
...
export default function MusicEditor({
  ...
  return (
    <div className="space-y-6 pt-2">
      ...
      {/* Auto-play Configuration Section */}
      <div className="rounded-xl border border-border/80 bg-card/60 p-4 sm:p-5 backdrop-blur-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border/60 pb-3">
          <div className="flex items-center gap-3">
            <div className="h-9 w-9 rounded-xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-center text-amber-500 shrink-0">
              <Volume2 className="h-4.5 w-4.5" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h4 className="font-semibold text-sm text-foreground">Auto-play Options</h4>
                <Badge
                  variant="outline"
                  className={`text-[10px] px-1.5 py-0 ${
                    autoPlayCover && autoPlayInvitation
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                Choose whether music begins playing on the cover screen or when opening the invitation.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1 bg-muted/60 p-1 rounded-lg border border-border/60 shrink-0">
            <button
              type="button"
              onClick={() => onAutoPlayChange?.({ autoPlayCover: true, autoPlayInvitation: true })}
              className={`text-[11px] px-2.5 py-1 rounded-md transition-all font-medium ${
                autoPlayCover && autoPlayInvitation
                  ? "bg-primary text-primary-foreground shadow-sm"
                  : "text-muted-foreground hover:text-foreground"
              }`}
              title="Plays on cover and continues seamlessly into invitation"
            >
              Cover & Invite
            </button>
            <button
              type="button"
              onClick={() => onAutoPlayChange?.({ autoPlayCover: false, autoPlayInvitation: true })}
              className={`text-[11px] px-2.5 py-1 rounded-md transition-all font-medium ${
                !autoPlayCover && autoPlayInvitation
                  ? "bg-primary text-primary-foreground shadow-sm"
                  : "text-muted-foreground hover:text-foreground"
              }`}
              title="Silent on cover; music swells when guest taps Open Invitation"
            >
              Invite Only
            </button>
            <button
              type="button"
              onClick={() => onAutoPlayChange?.({ autoPlayCover: true, autoPlayInvitation: false })}
              className={`text-[11px] px-2.5 py-1 rounded-md transition-all font-medium ${
                autoPlayCover && !autoPlayInvitation
                  ? "bg-primary text-primary-foreground shadow-sm"
                  : "text-muted-foreground hover:text-foreground"
              }`}
              title="Plays only on the cover screen"
            >
              Cover Only
            </button>
            <button
              type="button"
              onClick={() => onAutoPlayChange?.({ autoPlayCover: false, autoPlayInvitation: false })}
              className={`text-[11px] px-2.5 py-1 rounded-md transition-all font-medium ${
                !autoPlayCover && !autoPlayInvitation
                  ? "bg-destructive text-destructive-foreground shadow-sm"
                  : "text-muted-foreground hover:text-foreground"
              }`}
              title="Disable autoplay; guests must tap the floating music disc"
            >
              Off
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
          {/* Cover Screen Auto-play Toggle Card */}
          <div
            onClick={() => onAutoPlayChange?.({ autoPlayCover: !autoPlayCover, autoPlayInvitation })}
            className={`cursor-pointer rounded-lg border p-3.5 flex items-start justify-between gap-3 transition-all select-none ${
              autoPlayCover
                ? "border-primary/60 bg-primary/5 shadow-xs"
                : "border-border/70 bg-card/40 hover:bg-card/70 opacity-75"
            }`}
          >
            <div className="space-y-1 pr-2">
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold text-foreground">Auto-play on Cover Screen</span>
                <span
                  className={`inline-block h-2 w-2 rounded-full transition-colors ${
                    autoPlayCover ? "bg-emerald-400" : "bg-muted-foreground/40"
                  }`}
                />
              </div>
              <p className="text-[11px] text-muted-foreground leading-relaxed">
                Music plays immediately as soon as the guest views or touches the cover screen (traditional wedding gate / crest).
              </p>
            </div>
            <Switch
              checked={autoPlayCover}
              onCheckedChange={(checked) =>
                onAutoPlayChange?.({ autoPlayCover: checked, autoPlayInvitation })
              }
              onClick={(e) => e.stopPropagation()}
            />
          </div>

          {/* Invitation Page Auto-play Toggle Card */}
          <div
            onClick={() => onAutoPlayChange?.({ autoPlayCover, autoPlayInvitation: !autoPlayInvitation })}
            className={`cursor-pointer rounded-lg border p-3.5 flex items-start justify-between gap-3 transition-all select-none ${
              autoPlayInvitation
                ? "border-primary/60 bg-primary/5 shadow-xs"
                : "border-border/70 bg-card/40 hover:bg-card/70 opacity-75"
          ...
          >
            <div className="space-y-1 pr-2">
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold text-foreground">Auto-play on Invitation Page</span>
                <span
                  className={`inline-block h-2 w-2 rounded-full transition-colors ${
                    autoPlayInvitation ? "bg-emerald-400" : "bg-muted-foreground/40"
                  }`}
                />
              </div>
              <p className="text-[11px] text-muted-foreground leading-relaxed">
                Music plays automatically when the guest taps &quot;Open Invitation&quot; or when viewing the invitation page directly.
              </p>
            </div>
            <Switch
              checked={autoPlayInvitation}
              onCheckedChange={(checked) =>
                onAutoPlayChange?.({ autoPlayCover, autoPlayInvitation: checked })
              }
              onClick={(e) => e.stopPropagation()}
            />
          </div>
        ...
        {/* Dynamic Tip / Recommendation */}
        <div className="text-[11px] text-muted-foreground flex items-start sm:items-center gap-2 px-1 pt-0.5">
          <Sparkles className="h-3.5 w-3.5 text-amber-400 shrink-0 mt-0.5 sm:mt-0" />
          <span>
            {!autoPlayCover && autoPlayInvitation ? (
              <>
                <strong className="text-amber-400 font-medium">Dramatic Reveal mode active: </strong>
                The cover screen stays completely quiet, and your romantic music swells the exact instant your guest taps &quot;Open Invitation&quot;!
              </>
            ) : autoPlayCover && autoPlayInvitation ? (
              <>
                <strong className="text-foreground font-medium">Continuous Romance mode: </strong>
                Music welcomes guests upon arrival on the cover and continues playing uninterrupted into the ceremony invitation.
              </>
            ) : autoPlayCover && !autoPlayInvitation ? (
              <>
                <strong className="text-foreground font-medium">Cover Only: </strong>
                Music plays exclusively during the initial cover presentation.
              </>
            ) : (
              <>
                <strong className="text-foreground font-medium">Manual Playback: </strong>
                Guests can enjoy complete silence or tap the floating disc at the bottom-right corner whenever they want to listen.
              </>
            )}
          </span>
        </div>
